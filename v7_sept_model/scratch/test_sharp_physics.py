...
def test_sharp_physics():
    ...
    # Let's inspect different formulations of Expert 4 correction
    print("Testing variations of Expert 4 physical error reduction...")

    # We will subclass or monkeypatch forward
    original_forward = moe.forward

    # Test 1: Original
    moe.forward = original_forward
    d_orig = run_eval(moe, test_scenarios, scalers, device)
    print(f"Original: Overall={d_orig['overall']:.2f}m | Motorway={d_orig['motorway']:.2f}m | Roundabout={d_orig['roundabout']:.2f}m | SharpTurns={d_orig['sharp_turns']:.2f}m")

    # Test 2: Replace torch.sign(wyaw) with physical centripetal: w_cent = - alat / v_safe
    def forward_phys(x):
        ...
        # True physical lateral centripetal: w_cent = - alat / v
        w_cent4 = -(alat / v_safe) * 0.50
        has_turn_transient = (torch.abs(alat) > 1.4)
        # When turning transient occurs, combine gyro and clean centripetal
        wr4_p = torch.where(has_turn_transient, 0.50 * wr4 + 0.50 * w_cent4, wr4)
        xr4_p = torch.where(has_turn_transient, torch.clamp(xr4, max=vprev), xr4)
    ...
    moe.forward = forward_phys
    d_phys = run_eval(moe, test_scenarios, scalers, device)
    print(f"Test 2 (-alat/v): Overall={d_phys['overall']:.2f}m | Motorway={d_phys['motorway']:.2f}m | Roundabout={d_phys['roundabout']:.2f}m | SharpTurns={d_phys['sharp_turns']:.2f}m")
