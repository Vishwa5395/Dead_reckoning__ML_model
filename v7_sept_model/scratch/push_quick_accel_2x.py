...
for g_pos in np.arange(0.60, 1.10, 0.05):
    for g_neg in np.arange(0.20, 0.60, 0.05):
        for af_pos_th in [0.2, 0.3, 0.4, 0.5]:
            drifts = []
            for s in range(11, len(x_gps) - 10, 10):
                ...
                for k in range(10):
                    ...
                    if a_fwd[cur] > af_pos_th:
                        xr = max(xr, history_x[-1] + a_fwd[cur] * g_pos)
                    elif a_fwd[cur] < -0.3:
                        xr = min(xr, history_x[-1] + a_fwd[cur] * g_neg)
            ...
            if mean_d < best_drift:
                best_drift = mean_d
                best_cfg = (g_pos, g_neg, af_pos_th, drifts)

print(f"Optimal Quick Accel: g_pos={best_cfg[0]:.2f}, g_neg={best_cfg[1]:.2f}, af_th={best_cfg[2]:.2f} -> Drift = {best_drift:.2f}m")
