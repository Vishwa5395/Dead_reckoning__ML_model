...
for s in range(11, len(x_gps) - 10, 10):
    ...
    for k in range(10):
        ...
        wr = float(s_yo.inverse_transform([[o.item()]])[0, 0]) * 2.8

        if abs(a_lat[cur]) > 0.5 and history_x[-1] > 3.0:
...
print(f"best_supreme_roundabout.pth Mean Drift: {np.mean(drifts):.2f}m | Outages: {[round(x, 2) for x in drifts]}")
