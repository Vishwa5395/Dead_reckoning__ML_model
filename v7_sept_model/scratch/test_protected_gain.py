...
def main():
    ...
    print("Testing Protected Cornering Gain on Expert 4...")

    for alat_thresh in [1.2, 1.4, 1.6]:
        for gain_w in [1.5, 2.0, 2.5, 3.0, 3.5]:
            def forward_test(x):
                ...
                # Expert 4 (Sharp Turns) - PROTECTED AGAINST BRAKING
                is_sharp_corner = (torch.abs(alat) > alat_thresh) & (afwd > -0.6) & (vprev < 22.0)
                w_cent4 = torch.sign(wyaw) * (torch.abs(alat) / v_safe) * 0.65
                wr4_boosted = wr4 * gain_w
                wr4_p = torch.where(is_sharp_corner, 0.50 * wr4_boosted + 0.50 * w_cent4, wr4)
                xr4_p = torch.where(is_sharp_corner, torch.clamp(xr4, max=vprev - 0.3), xr4)
            ...
            print(f"alat_th={alat_thresh}, gain={gain_w} -> Sharp: {res['sharp_turns']:5.2f}m | Overall: {res['overall']:5.2f}m | Mot: {res['motorway']:4.2f}m | RB: {res['roundabout']:5.2f}m | HB: {res['hard_brake']:5.2f}m | QA: {res['quick_accel']:5.2f}m", flush=True)
