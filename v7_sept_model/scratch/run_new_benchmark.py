...
with open('v7_sept_model/data/test_scenarios_v7.pkl', 'rb') as f: test_scenarios = pickle.load(f)

s_X = scalers['X']; s_yd = scalers['y_disp']; s_yo = scalers['y_ori']

s1_mot = StraightSpecialistS1().to(device)
s1_mot.load_state_dict(torch.load('v3_pino_dr/checkpoints/best_model.pth', map_location=device)['model_state_dict'])
s1_mot.eval()
...
ckpt_rb = torch.load('v7_sept_model/checkpoints/best_supreme_roundabout.pth', map_location=device)
...
total_drifts = []
print("=" * 90)
print("BENCHMARK EVALUATION: 5 SUPREME SPECIALISTS (SHARP TURNS & HARD BRAKE ADVANCED)")
print("=" * 90)
...
for scen in SCENARIOS:
    ...
    results[scen] = mean_drift

overall_mean = float(np.mean(total_drifts))
imprv_ov = ((29.55 - overall_mean) / 29.55) * 100.0
print("-" * 90)
print(f'OVERALL MEAN:  {overall_mean:6.2f}m | Best Repo = 29.55m | Imprv vs Repo = {imprv_ov:+6.1f}%')
print("=" * 90)
