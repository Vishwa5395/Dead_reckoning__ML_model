...
for g_pos in np.arange(0.2, 0.9, 0.1):
    for g_neg in np.arange(0.0, 0.9, 0.1):
        drifts = []
        ...
        if mean_d < best_drift:
            best_drift = mean_d
            best_params = (g_pos, g_neg, drifts)

print(f"Optimal Quick Accel: g_pos={best_params[0]:.2f}, g_neg={best_params[1]:.2f} -> Mean Drift = {best_drift:.2f}m")
print(f"Outages: {[round(x, 2) for x in best_params[2]]}")
