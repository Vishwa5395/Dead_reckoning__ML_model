...
for g_brake in np.arange(0.0, 1.0, 0.05):
    drifts = []
    for j in journeys:
        x_gps = j["x_gps"]
        w_gps = j["w_gps"]
        ...
        for s in range(11, len(x_gps) - 10, 10):
            ...
            for k in range(10):
                ...
                with torch.no_grad():
                    d, o, _ = s1_mot(torch.tensor(win_s4, dtype=torch.float32, device=device))
                xr = float(s_yd.inverse_transform([[d.item()]])[0, 0])
                wr = float(s_yo.inverse_transform([[o.item()]])[0, 0])

                if a_fwd[cur] < -0.5 and g_brake > 0:
                    xr = min(xr, history_x[-1] + a_fwd[cur] * g_brake)
...
print(f"Optimal Hard Brake: g_brake={best_g:.2f} -> Mean Drift = {best_drift:.2f}m")
