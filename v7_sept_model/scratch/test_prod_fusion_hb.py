...
import numpy as np
import torch
from v7_sept_model.src.models_v7 import StraightSpecialistS1, TurningSpecialistS2, ProductionDualSpecialistFusion
...
s1 = StraightSpecialistS1(in_channels=4).to(device)
...
fusion = ProductionDualSpecialistFusion(model_s1=s1, model_s2=s2, tau_enter=0.011, tau_exit=0.007, fade_time_s=0.5).to(device)
fusion.eval()
...
for j in journeys:
    ...
    for s in range(11, len(x_gps) - 10, 10):
        ...
        for k in range(10):
            ...
            with torch.no_grad():
                d, o, z, alpha, _ = fusion(
                    torch.tensor(win_s6, dtype=torch.float32, device=device),
...
print(f"ProductionDualSpecialistFusion on Hard Brake: {np.mean(drifts):.2f}m")
