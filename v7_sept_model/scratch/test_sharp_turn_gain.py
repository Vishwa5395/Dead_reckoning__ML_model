...
def main():
    ...
    print("Testing Expert 4 Angular Gain and Centripetal Physics...")

    # Grid search over angular gain on wr4 and speed coupling
    for gain_w in [1.0, 1.5, 2.0, 2.5, 3.0]:
        for speed_damp in [0.0, 0.2, 0.4]:
            def forward_test(x):
                ...
                # When turning dynamics are active:
                is_cornering = (torch.abs(wyaw) > 0.035) | (torch.abs(alat) > 1.2)
                wr4_p = torch.where(is_cornering, wr4 * gain_w, wr4)
                xr4_p = torch.where(is_cornering, torch.clamp(xr4, max=vprev - speed_damp), xr4)
            ...
            moe.forward = forward_test
            res = run_eval(moe, test_scenarios, scalers, device)
            print(f"gain_w={gain_w:3.1f}, speed_damp={speed_damp:3.1f} | Overall: {res['overall']:5.2f}m | Mot: {res['motorway']:4.2f}m | RB: {res['roundabout']:5.2f}m | QA: {res['quick_accel']:5.2f}m | HB: {res['hard_brake']:5.2f}m | Sharp: {res['sharp_turns']:5.2f}m")
