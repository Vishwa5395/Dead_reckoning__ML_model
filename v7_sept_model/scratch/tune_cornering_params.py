...
def main():
    ...
    print("Fine-tuning Expert 4 cornering parameters...")

    best_overall = 27.96
    best_sharp = 35.94
    best_params = None

    # Test combinations around the current baseline (1.6 threshold, 0.65 centripetal, 0.5 max speed damp)
    for alat_thresh in [1.4, 1.5, 1.6, 1.7, 1.8]:
        for cent_gain in [0.45, 0.55, 0.65, 0.75, 0.85]:
            for wr4_ratio in [0.40, 0.50, 0.60, 0.70]:
                for speed_clamp in [-0.2, -0.5, -0.8]:
                    def forward_param(x):
                    ...
                    if res["sharp_turns"] < best_sharp or res["overall"] < best_overall:
                        ...
                        best_params = (alat_thresh, cent_gain, wr4_ratio, speed_clamp)
                        print(f"NEW BEST! alat_thresh={alat_thresh}, cent_gain={cent_gain}, wr4_ratio={wr4_ratio}, speed_clamp={speed_clamp} -> Sharp: {res['sharp_turns']:.2f}m, Overall: {res['overall']:.2f}m, Motorway: {res['motorway']:.2f}m, RB: {res['roundabout']:.2f}m, HB: {res['hard_brake']:.2f}m, QA: {res['quick_accel']:.2f}m")

    print("\nSearch complete. Best params:", best_params)
