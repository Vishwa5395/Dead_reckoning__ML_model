...
for w_scale in np.arange(2.0, 4.2, 0.2):
    for cent_gain in np.arange(0.3, 0.9, 0.1):
        for blend_c in np.arange(0.2, 0.7, 0.1):
            drifts = []
            for s in range(11, len(x_gps) - 10, 10):
                ...
                for k in range(10):
                    ...
                    wr = float(s_yo.inverse_transform([[o.item()]])[0, 0]) * w_scale

                    if abs(a_lat[cur]) > 0.4 and history_x[-1] > 2.5:
                        w_cent = -np.sign(a_lat[cur]) * abs(a_lat[cur]) / history_x[-1] * cent_gain
                        wr = (1.0 - blend_c) * wr + blend_c * w_cent
            ...
            if mean_d < best_drift:
                best_drift = mean_d
                best_cfg = (w_scale, cent_gain, blend_c, drifts)

print(f"Optimal Roundabout: w_scale={best_cfg[0]:.2f}, cent_gain={best_cfg[1]:.2f}, blend_c={best_cfg[2]:.2f} -> Drift = {best_drift:.2f}m")
