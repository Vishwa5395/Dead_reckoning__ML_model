...
for dec_gain in np.arange(0.3, 0.9, 0.05):
    for w_damping in [0.0, 0.05, 0.1, 0.2]:
        drifts = []
        for j in journeys:
            x_gps = j["x_gps"]
            w_gps = j["w_gps"]
        ...
        if mean_d < best_drift:
            best_drift = mean_d
            best_cfg = (dec_gain, w_damping, drifts)

print(f"Optimal Hard Brake: dec_gain={best_cfg[0]:.2f}, w_damping={best_cfg[1]:.2f} -> Drift = {best_drift:.2f}m")
