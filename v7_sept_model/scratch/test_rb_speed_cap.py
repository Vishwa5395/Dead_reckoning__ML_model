...
for max_v_rb in [8.0, 9.0, 10.0, 11.0, 12.0, 15.0]:
    drifts = []
    for s in range(11, len(x_gps) - 10, 10):
        ...
        for k in range(10):
            ...
            # Physical speed cap during roundabout curve
            if abs(wr) > 0.10: # turning sharply in roundabout
                xr = min(xr, max_v_rb)
    ...
    print(f"max_v_rb={max_v_rb:5.1f}m/s -> Mean Drift: {np.mean(drifts):.2f}m | Outages: {[round(x, 2) for x in drifts]}")
