...
m = StraightSpecialistS1().to(device)
...
for j in journeys:
    ...
    for s in range(11, len(x_gps) - 10, 10):
        ...
        for k in range(10):
            ...
            with torch.no_grad():
                d, o, _ = m(torch.tensor(win_s4, dtype=torch.float32, device=device))
...
print(f"v3 Base on Hard Brake Mean Drift: {np.mean(drifts):.2f}m")
print(f"Outages: {[round(x, 2) for x in drifts]}")
