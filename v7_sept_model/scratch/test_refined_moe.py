...
SHARP_CKPT = CKPT_DIR / "best_supreme_sharp_turns.pth"

def test_refined_routing():
    ...
    # Load sharp turns specialist into exp4
    sharp_state = torch.load(SHARP_CKPT, map_location=device, weights_only=False)["model_state_dict"]
    exp4_sharp = TurningExpertNetwork(in_channels=6).to(device)
    exp4_sharp.load_state_dict(sharp_state)
    exp4_sharp.eval()

    # Base exp4 (v4-D)
    exp4_base = TurningExpertNetwork(in_channels=6).to(device)
    exp4_base.load_state_dict(torch.load(V4_D_CKPT, map_location=device, weights_only=False)["model_state_dict"])
    exp4_base.eval()

    print("Evaluating with physical regime separation...")

    # Let's test a forward pass that separates cruising from acute turning:
    def custom_forward(x):
        ...
        # Check if acute turn is occurring in the window
        # In normalized space, channel 1 is wyaw, channel 2 is alat
        wyaw_mean = torch.mean(torch.abs(x[:, :, 1] - 0.5000), dim=1, keepdim=True) * 2.0
        alat_mean = torch.mean(torch.abs(x[:, :, 2] - 0.5241), dim=1, keepdim=True) * 11.95
        ...
        # Acute turning expert:
        # Use trained sharp specialist when turning is active, otherwise use base
        d4_sharp, o4_sharp, z4_sharp = exp4_sharp(x)
        d4_base, o4_base, z4_base = exp4_base(x)

        is_turning = (wyaw_mean > 0.035) | (alat_mean > 1.2)
        d4_raw = torch.where(is_turning, d4_sharp, d4_base)
        o4_raw = torch.where(is_turning, o4_sharp, o4_base)
        z4 = torch.where(is_turning, z4_sharp, z4_base)
        ...
        # In sharp turns, decelerate into turn apex:
        xr4_p = torch.where(is_turning, torch.clamp(xr4, max=vprev + 0.1), xr4)
        wr4_p = wr4
    ...
    moe.forward = custom_forward
    res = evaluate_moe(moe, test_scenarios, scalers, device)
    print("\n--- Results with Conditional Sharp Specialist ---")
