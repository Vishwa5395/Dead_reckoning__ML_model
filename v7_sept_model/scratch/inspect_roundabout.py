...
for s in range(11, len(x_gps) - 10, 10):
    ...
    print(f"\n==================== ROUNDABOUT OUTAGE {outage_idx} (start_idx={s}) ====================")
    for k in range(10):
        cur = s + k
        ...
        w_cent = 0.0
        if abs(a_lat[cur]) > 1.8 and history_x[-1] > 4.0:
            w_cent = -np.sign(a_lat[cur]) * abs(a_lat[cur]) / history_x[-1] * 0.5
            wr_fused = 0.65 * wr + 0.35 * w_cent
        else:
            wr_fused = wr
        ...
        print(f"k={k:02d} | v_gt={x_gps[cur]:5.2f}, v_p={xr:5.2f} | w_gt={np.degrees(w_gps[cur]):+6.2f}°, w_imu={np.degrees(w_yaw[cur]):+6.2f}°, w_p={np.degrees(wr):+6.2f}°, w_cent={np.degrees(w_cent):+6.2f}° | HeadErr={head_err_deg:5.2f}°, Drift={step_drift:5.2f}m")
