...
for w_gain in [0.0, 0.05, 0.10, 0.15]:
    for dec_gain in [0.0, 0.1, 0.2, 0.3]:
        drifts = []
        for j in journeys:
            x_gps = j["x_gps"]
            w_gps = j["w_gps"]
            ...
            for s in range(11, len(x_gps) - 10, 10):
                ...
                for k in range(10):
                    ...
                    if a_fwd[cur] < -1.0 and dec_gain > 0:
        ...
        if mean_d < best_drift:
            best_drift = mean_d
            best_cfg = (w_gain, dec_gain, drifts)

print(f"Optimal Sharp Turns: w_gain={best_cfg[0]:.2f}, dec_gain={best_cfg[1]:.2f} -> Drift = {best_drift:.2f}m")
