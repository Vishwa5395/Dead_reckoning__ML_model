...
# Precompute windows so testing 20 gains takes 0.5 seconds
precomputed_steps = []
...
print(f"Loaded {len(precomputed_steps)} outages.")

for gain in [0.0, 0.02, 0.05, 0.08, 0.10, 0.12, 0.15, 0.20, 0.25, 0.30, 0.40, 0.50, 0.70, 1.00]:
    drifts = []
    for d in precomputed_steps:
        ...
        for k in range(10):
            ...
            with torch.no_grad():
                ...
                wr = float(s_yo.inverse_transform([[o_p.item()]])[0, 0]) * gain
    ...
    print(f"Gain = {gain:4.2f} -> Sharp Turns Mean Drift = {np.mean(drifts):6.2f} m")
