...
for a_thresh in [-0.1, -0.2, -0.3, -0.5]:
    for g_dec in np.arange(0.1, 0.9, 0.05):
        drifts = []
        for j in journeys:
            x_gps = j["x_gps"]
            w_gps = j["w_gps"]
            ...
            for s in range(11, len(x_gps) - 10, 10):
                ...
                for k in range(10):
                    ...
                    if a_fwd[cur] < a_thresh:
                        xr = min(xr, max(0.0, history_x[-1] + a_fwd[cur] * g_dec))
        ...
        if mean_d < best_drift:
            best_drift = mean_d
            best_params = (a_thresh, g_dec, drifts)

print(f"Optimal Hard Brake: a_thresh={best_params[0]:.2f}, g_dec={best_params[1]:.2f} -> Mean Drift = {best_drift:.2f}m")
