...
def main():
    ...
    res_orig = run_eval(moe, test_scenarios, scalers, device)
    for k, v in res_orig.items():
        print(f"  {k:15s}: {v:6.2f}m")

    # Test 2: Fix line 465 (wr3_p = torch.where(is_standstill, torch.zeros_like(wr3), wr3))
    def forward_fix_line465(x):
        ...
        # FIX: Only zero heading when at complete standstill; do not multiply by 0.05 during moving braking!
        wr3_p = torch.where(is_standstill, torch.zeros_like(wr3), wr3)
    ...
    moe.forward = forward_fix_line465
    print("\n--- Test 2: Fixing wr3 * 0.05 clamp ---")
    res_fix = run_eval(moe, test_scenarios, scalers, device)
    for k, v in res_fix.items():
        print(f"  {k:15s}: {v:6.2f}m")
