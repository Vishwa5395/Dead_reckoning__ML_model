...
preprocess_v4.py
----------------
Production-grade data preparation for PINO-DR v4 (Turn-Focused).

Key improvements over v3:
- 6-channel temporal tensor: [a_fwd, w_yaw, a_lat_measured, v_prev, yaw_accel, centripetal_residual]
- 10 Hz smoothed yaw acceleration using Savitzky-Golay filtering before 1 Hz downsampling
- Explicit centripetal residual feature: a_lat - v_prev * w_yaw (using predicted speed at inference)
- Exact frozen V3 trip-level split (50 train / 9 val / 13 test journeys, zero temporal leakage)
- Calibrated turn threshold tau_turn fitted strictly on train split
...
# Named test scenario journey tags (identical to v3 / Onyekpe et al.)
TEST_SCENARIOS = {
    'motorway': ['Vw12'],
    'roundabout': ['Vta11'],
    'quick_accel': ['Vta12'],
...
def process_journey_v4(s_path, v_path, name="journey"):
    """
    Processes one synchronized journey into 1-second intervals.
    Returns enriched arrays including lateral acceleration and smoothed yaw acceleration.
    """
    ...
    w_yaw_aligned = yaw_sign * w_vert  # 10 Hz aligned yaw rate (rad/s)

    # ── Yaw acceleration via Savitzky-Golay differentiation at 10 Hz (dt = 0.1s) ──
    w_len = len(w_yaw_aligned)
    sg_win = 5 if w_len >= 5 else (w_len if w_len % 2 == 1 else w_len - 1)
    if sg_win >= 3:
        w_yaw_accel_10hz = savgol_filter(w_yaw_aligned, window_length=sg_win, polyorder=2, deriv=1, delta=0.1)
    else:
        w_yaw_accel_10hz = np.gradient(w_yaw_aligned, 0.1)
...
def create_v4_windows(journeys, window_size=WINDOW_SIZE, is_train=True, noise_std=NOISE_STD):
    """
    Build (N, W, 6) temporal windows + targets.
    ...
    Channels:
      ...
      4: w_yaw_accel (smoothed yaw acceleration / turn initiation signal)
      5: centripetal_residual (a_lat - v_prev * w_yaw, explicit interaction feature)
    """
    X_list = []
    y_disp_list = []
    y_ori_list = []
    ...
    for j in journeys:
        ...
        for t in range(window_size, n):
            ...
            window = np.stack([
                ch_a_fwd, ch_w_yaw, ch_a_lat, ch_v_prev, ch_w_accel, ch_centripetal
            ], axis=-1)  # (10, 6)
    ...
    X = np.array(X_list, dtype=np.float32)             # (N, 10, 6)
...
def main():
    ...
    print(f"[v4] Found {len(pairs)} synchronized (S, V) pairs.")

    all_journeys = []
    total_seconds = 0
    for key in sorted(pairs.keys()):
        s_p, v_p = pairs[key]
        j = process_journey_v4(s_p, v_p, name=key)
        if j is not None:
            all_journeys.append(j)
            total_seconds += len(j['x_gps'])

    print(f"[v4] Processed {len(all_journeys)} journeys, {total_seconds:,} seconds total.")

    # ── Trip-level split BEFORE windowing (Strictly preserving v3 split, seed=42) ──
    test_named_tags = set()
    ...
    print(f"\n[v4] Frozen Split: {len(train_journeys)} train journeys / "
    ...
    print("[v4] Verified: zero journey overlap across splits.")
    ...
    # Composite turn score: sqrt((w / std_w)^2 + (w_dot / std_w_dot)^2)
    turn_scores_train = np.sqrt((all_train_w_yaw / std_w_yaw)**2 + (all_train_w_accel / std_w_accel)**2)
    tau_turn = float(np.percentile(turn_scores_train, 75))  # 75th percentile marks significant turning
    print(f"\n[v4] Calibrated Turn Statistics (Train Split Only):")
    print(f"  std(w_yaw):       {std_w_yaw:.4f} rad/s")
    print(f"  std(w_yaw_accel): {std_w_accel:.4f} rad/s²")
    print(f"  tau_turn (75th percentile): {tau_turn:.4f}")

    # ── Generate windows AFTER split ──
    print("\n[v4] Windowing AFTER split (W=10, 6 channels)...")
    X_tr, y_d_tr, y_o_tr, y_z_tr = create_v4_windows(train_journeys, is_train=True)
    X_va, y_d_va, y_o_va, y_z_va = create_v4_windows(val_journeys, is_train=False)
    X_te, y_d_te, y_o_te, y_z_te = create_v4_windows(test_journeys, is_train=False)
    ...
    n_tr = X_tr.shape[0]
    X_tr_flat = X_tr.reshape(n_tr, -1)  # (N, 60)
    ...
    with open(DATA_DIR / "scalers_v4.pkl", "wb") as f:
        pickle.dump(scalers, f)

    np.savez_compressed(
        DATA_DIR / "dataset_splits_v4.npz",
    ...
    with open(DATA_DIR / "test_scenarios_v4.pkl", "wb") as f:
        pickle.dump(test_scenario_journeys, f)

    meta = {
        "channels": ["a_fwd", "w_yaw", "a_lat_measured", "v_prev", "w_yaw_accel", "centripetal_residual"],
        "num_channels": 6,
    ...
    with open(DATA_DIR / "metadata_v4.json", "w") as f:
        json.dump(meta, f, indent=2)

    sz = (DATA_DIR / "dataset_splits_v4.npz").stat().st_size / 1024
    print(f"\n[v4] Saved to {DATA_DIR}")
    print(f"     dataset_splits_v4.npz ({sz:.1f} KB)")
    print(f"     scalers_v4.pkl, test_scenarios_v4.pkl, metadata_v4.json")
